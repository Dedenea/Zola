...
const GroupTab = ({navigation}) =>{
    ...
    useEffect(() => {
        fetch('https://617fa6a3055276001774fb98.mockapi.io/Test2')
    ...
    return (
        <SafeAreaView>
            {isLoading ? <ActivityIndicator/>:(
                ...
                renderItem={({item})=>(
                <Pressable onPress={()=>navigation.navigate('ChatOneOne',{id:item.id,name:item.name,image:item.image,text:item.text,time:item.time})} style={styles.container}>
                    <View style={styles.leftContainer}>
                        ...
                        <Text style={styles.aa}>{item.time}</Text>
                    </View>
                </Pressable>  
                )}
            /></View>)}
